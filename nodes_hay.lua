...
-- If hades_core:dirt_with_grass is digged while wielding a pitchfork, it will
-- turn into dirt and get some hay placed above it.
-- The hay will disappear (decay) after a couple of minutes.
if(     minetest.registered_items["hades_core:dirt_with_grass"]
    and minetest.registered_tools["hades_cottages:pitchfork"]) then
  minetest.override_item("hades_core:dirt_with_grass", {
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
		...
		if(    not( wielded )
		    ...
		    or (wielded:get_name()~="hades_cottages:pitchfork")) then
			return
		end
		...
		minetest.add_node(  pos_above, {name="hades_cottages:hay_mat", param2=math.random(2,25)}) 
...
minetest.register_node("hades_cottages:hay_mat", {
	drawtype = "nodebox",
	paramtype2 = "leveled",
	...
	after_place_node = function(pos, placer, itemstack, pointed_thing)
		minetest.swap_node( pos, {name="hades_cottages:hay_mat", param2=math.random(2,25)})
	end,
	on_timer = function(pos, elapsed)
		local node = minetest.get_node(pos)
		if( node and node.name=="hades_cottages:hay_mat") then
...
minetest.register_node("hades_cottages:hay", {
	description = S("Hay"),
	tiles = {cottages.straw_texture.."^[multiply:#88BB88"},
...
minetest.register_node("hades_cottages:hay_bale", {
	drawtype = "nodebox",
...
minetest.register_craft({
	output = "hades_cottages:hay_mat 9",
	recipe = {
		{"hades_cottages:hay"},
	},
})
...
minetest.register_craft({
	output = "hades_cottages:hay",
	recipe = {
		{"hades_cottages:hay_mat", "hades_cottages:hay_mat", "hades_cottages:hay_mat"},
		{"hades_cottages:hay_mat", "hades_cottages:hay_mat", "hades_cottages:hay_mat"},
		{"hades_cottages:hay_mat", "hades_cottages:hay_mat", "hades_cottages:hay_mat"},
	},
})
...
minetest.register_craft({
	output = "hades_cottages:hay",
	recipe = {{"hades_cottages:hay_bale"}},
})

minetest.register_craft({
	output = "hades_cottages:hay_bale",
	recipe = {{"hades_cottages:hay"}},
})
