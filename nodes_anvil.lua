...
minetest.register_alias("hades_cottages:hammer", "cottages:hammer")

local enable_anvil = minetest.settings:get_bool("cottages_enable_anvil", false)
...
if enable_anvil then
	...
	--		  or not( inv:is_empty("sample"))
		...
		allow_metadata_inventory_put = function(pos, listname, index, stack, player)
			...
			if( listname=='hammer' and stack and stack:get_name() ~= 'hades_cottages:hammer') then
				return 0;
			end
		...
		on_punch = function(pos, node, puncher)
			...
			if( not( wielded ) or not( wielded:get_name() ) or wielded:get_name() ~= 'hades_cottages:hammer') then
				return;
			end
	...
	if ( minetest.get_modpath("castle") ~= nil ) then
		...
		output = "castle:anvil",
		recipe = {
			 {'hades_cottages:anvil'},
			},
		}) 
	end

	minetest.register_craft({
		output = "hades_cottages:hammer",
		recipe = {
									{cottages.craftitem_steel},
									{'hades_cottages:anvil'},
	...
	minetest.register_craft({
		...
		recipe = {
									{cottages.craftitem_steel},
									{cottages.craftitem_steel_block},
									{cottages.craftitem_stick} }
	})
end
